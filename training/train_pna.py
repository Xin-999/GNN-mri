#!/usr/bin/env python
"""
Train a PNA (Principal Neighbourhood Aggregation) regressor across CV folds.
PNA is a newer GNN architecture that aggregates multiple statistics per node.

Examples:
  # Full argument example
  python training/train_pna.py \
    --fold_dir data/folds_data \
    --output_dir results/pna \
    --run_name pna_full \
    --device cuda \
    --seed 42 \
  ...
  python training/train_pna.py --fold_name graphs_outer1_inner1
...
Arguments:
  ...
  --num_layers: number of PNAConv layers
  --dropout: dropout for hidden layers/head
  --aggregators: list of aggregator functions for PNAConv
  --scalers: list of degree scalers for PNAConv
  --towers: number of towers (hidden_dim must be divisible by towers)
  --pre_layers: number of pre-MLP layers inside PNAConv
  --post_layers: number of post-MLP layers inside PNAConv
  --pool: mean | attention
...
from torch_geometric.nn import GlobalAttention, PNAConv, global_mean_pool
...
class PNARegressor(nn.Module):
    def __init__(
        ...
        if hidden_dim % towers != 0:
            raise ValueError("hidden_dim must be divisible by towers")
...
def compute_degree_histogram(graphs: List) -> torch.Tensor:
    max_deg = 0
    for g in graphs:
        num_nodes = g.num_nodes if g.num_nodes is not None else g.x.size(0)
        deg = degree(g.edge_index[1], num_nodes=num_nodes, dtype=torch.long)
    ...
    for g in graphs:
        ...
        if deg.numel() == 0:
            continue
        deg_hist += torch.bincount(deg, minlength=deg_hist.numel())
...
def train_fold(
    ...
    deg_hist = compute_degree_histogram(train_graphs)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train PNA across folds")
    ...
    parser.add_argument("--aggregators", nargs="+", default=["mean", "max", "min", "std"])
    parser.add_argument("--scalers", nargs="+", default=["identity", "amplification", "attenuation"])
    ...
    for fold_path in fold_paths:
        print(f"Training PNA on {fold_path.name}")
    ...
    results_path = output_dir / "pna_cv_results.json"
