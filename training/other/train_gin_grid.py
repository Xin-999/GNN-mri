...
Train an interpretable GIN-based GNN on Movie-HCP Ledoit-Wolf graphs
with a small hyperparameter grid search.

- Loads all CV folds (graphs_outer*_inner*.pkl) from data/folds_data.
- Flattens [subject, window] arrays into a list of PyG Data objects.
- Drops padding windows (g.pad == True).
- Attaches subject_id to each window graph (for subject-level metrics).
- For each config in HYPERPARAM_GRID:
    - Trains a GIN regressor across all folds.
...
- Across ALL configs + folds, keeps ONLY ONE best model:
    ...
    - Saves weights to: results_gin_grid/gin_best_overall.pt
    - Saves metadata to: results_gin_grid/gin_best_overall_meta.json
"""
...
from torch_geometric.nn import GINConv, GlobalAttention, global_mean_pool

# ----------------------------
# 1. Config & hyperparameter grid
# ----------------------------

OUTPUT_DIR = "results_gin_grid"
...
SEED = 42
BATCH_SIZE = 32
EPOCHS = 40            # a bit more, but early stopping will stop earlier
...
# Hyperparameter grid for GIN:
# - hidden_dim: 32 vs 64
# - num_layers: 2 vs 3
# - lr: 1e-3 vs 5e-4
# - weight_decay: 5e-4 or 1e-3
# - layer_dropout: 0.4–0.6 (on GIN layers)
# - mlp_dropout: 0.4–0.6 (on final MLP)
HYPERPARAM_GRID = [
    {
        "hidden_dim": 32,
        "num_layers": 2,
...
# ----------------------------
# 3. GIN model (with dropout)
# ----------------------------

def make_mlp(in_dim: int, out_dim: int, hidden_dim: int):
    """Small 2-layer MLP used inside GINConv."""
...
class GINRegressor(nn.Module):
    """
    GIN-based regressor:
    - Several GINConv layers (node-level).
    - Dropout after each layer for regularisation.
    - Global attention or mean pooling over nodes.
    - MLP head with dropout for regression.

    Still trained at window-level (each graph = one window),
    but early stopping & "best model" are based on subject-level MSE
    (after averaging windows per subject).
    """
...
def main():
    ...
    best_overall_model_path = os.path.join(OUTPUT_DIR, "gin_best_overall.pt")
    best_overall_meta_path = os.path.join(OUTPUT_DIR, "gin_best_overall_meta.json")
    ...
    for cfg_idx, cfg in enumerate(HYPERPARAM_GRID):
        ...
        for fold_path in fold_files:
            ...
            # LR scheduler: reduce LR when val_subj MSE plateaus
            ...
            for epoch in range(1, EPOCHS + 1):
                ...
                # step scheduler on subject-level val MSE
                scheduler.step(val_mse_subj)
            ...
            if val_mse_subj < best_overall_val_mse:
                ...
                best_overall_info = {
                    ...
                    "test_subj_mse": float(test_subj_mse),
                    "test_subj_r": float(test_subj_r),
                }
                ...
                print(
                    f"*** New BEST OVERALL GIN model (cfg{cfg_idx}, fold {fold_name}) "
            ...
            cfg_test_mse_subj_list.append(test_subj_mse)
    ...
    summary_path = os.path.join(OUTPUT_DIR, "gin_grid_summary.json")
    ...
    if best_overall_info is not None:
        print("\n==============================")
        print("GLOBAL BEST GIN MODEL (lowest val_subj MSE across ALL configs+folds):")
